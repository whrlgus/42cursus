...
int compare(t_sprite *lhs, t_sprite *rhs){
	...
	lhs_dist = pow(lhs->pos.x - g_cub.player.pos.x, 2) + pow(lhs->pos.y - g_cub.player.pos.y, 2);
	rhs_dist = pow(rhs->pos.x - g_cub.player.pos.x, 2) + pow(rhs->pos.y - g_cub.player.pos.y, 2);
...
void calc_draw_sprite_info(t_draw_sprite_info *info, t_sprite *sprite)
{
	...
	info->transformX = info->invDet * (g_cub.player.dir.y * info->spriteX - g_cub.player.dir.x * info->spriteY);
	info->transformY = info->invDet * (-g_cub.player.plane.y * info->spriteX + g_cub.player.plane.x * info->spriteY);
	info->spriteScreenX = (int)((info->w / 2) * (1 + info->transformX / info->transformY));
	...
	info->drawEndX = ft_min(info->w-1, info->spriteWidth / 2 + info->spriteScreenX);
...
void draw_ele(t_sprite *sprite) {
	...
	while (++stripe < info.drawEndX)
	{
		...
		if(stripe > 0 && stripe < info.w && info.transformY > 0 && info.transformY < g_cub.zbuf[stripe])
		{
			y = info.drawStartY - 1;
			while (++y < info.drawEndY)
			{
				...
				info.color = g_cub.texture[sprite->tex_num].data[info.texWidth * info.texY + info.texX];
