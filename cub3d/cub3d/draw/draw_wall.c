...
void calc_draw_info(t_draw_info *info, t_cub *cub, int x)
{
	...
	if (info->ray.side)
	{
		...
		info->wall_x = cub->player.pos.x + info->perp_wall_dist * info->ray.dir.x;
	}
	else
	{
		info->tex_n = info->ray.dir.x > 0 ? 2 : 3;
		info->wall_x = cub->player.pos.y + info->perp_wall_dist * info->ray.dir.y;
	}
...
void calc_draw_tex_info(t_draw_tex_info *tex_info, t_cub *cub, t_draw_info *info){
	...
	tex_info->tex_pos = (info->draw_s - cub->window.height / 2 + info->line_h / 2) * tex_info->step;
}
...
int get_tex_color(t_draw_tex_info *tex_info, t_cub *cub, t_draw_info *info){
	...
	texY = (int)tex_info->tex_pos & (tex_info->tex_h - 1);
	pos = tex_info->tex_h * texY + tex_info->tex_x;
